...
- 发布方式为 rsync 覆盖文件（非 git pull）。小型优化改动无需逐次确认，可直接按「测试站 → 生产站 → git 提交推送」走完。
  - 测试站：`root@38.90.15.5:11519` → `/www/wwwroot/pp.lab19.top/`（域名 pp.19lab.top，库 ppmart2_test）
  - 生产站：同一台机 → `/www/wwwroot/store.19lab.top/`（域名 store.19lab.top，库 ppmart2）
  - 只同步改动到的文件，不要整目录覆盖；同步前先比对 md5，确认服务端基线与本地一致。
