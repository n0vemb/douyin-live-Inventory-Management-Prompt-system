...
$stmt = $pdo->prepare("UPDATE todo_items SET status = 'done', completed_by = ?, completion_detail = ?, completed_at = NOW() WHERE id = ? AND store_id = ? AND shop_id = ?");
$stmt->execute([$userId, $detail, $id, $storeId, $shopId]);
