...
try {
    $stmt = $pdo->prepare('SELECT * FROM pos_orders WHERE id = ? AND store_id = ?' . ($shopId ? ' AND shop_id = ?' : ''));
    ...
    try {
        $upd = $pdo->prepare(
            "UPDATE pos_orders SET pay_status = 'paid', paid_at = NOW()
             WHERE id = ? AND store_id = ? AND outbound_status = 'pending' AND pay_status = 'pending'" . ($shopId ? " AND shop_id = ?" : "")
        );
        $upd->execute($shopId ? [$orderId, $storeId, $shopId] : [$orderId, $storeId]);
        if ($upd->rowCount() === 0) {
            $pdo->rollBack();
