...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card-name {
            font-size: clamp(16px, 2vw, 32px); font-weight: 800; color: var(--text);
            white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
            margin-bottom: 6px; line-height: 1.1; flex-shrink: 0;
        }
        .sku-list { display: flex; flex-direction: column; gap: 2px; flex: 1; justify-content: center; overflow: hidden; }
        .sku-row {
            display: flex; align-items: center; gap: 8px;
            padding: 3px 0; border-top: 1px solid rgba(42,42,58,0.3);
            flex-shrink: 0;
        }
        .sku-row:first-child { border-top: none; }
        .sku-badge {
            font-size: clamp(10px, 1.1vw, 16px); padding: 3px 8px; border-radius: 4px;
            font-weight: 700; white-space: nowrap; flex-shrink: 0;
        }
        .sku-price {
            font-size: clamp(18px, 2.2vw, 36px); font-weight: 700; color: var(--success);
            margin-left: auto; white-space: nowrap; flex-shrink: 0;
        }
        .sku-stock {
            font-size: clamp(20px, 2.4vw, 44px); font-weight: 800; min-width: 36px; text-align: right;
...
<body>
    ...
    <script>
        ...
        function renderGrid() {
            ...
            products.sort(function(a, b) { return getDisplayName(a).length - getDisplayName(b).length; });
            ...
            var cols = Math.max(1, Math.floor((w - 12) / 300));
            ...
            grid.innerHTML = products.map(function(p) {
                ...
                return '<div class="product-card" data-barcode="' + (p.barcode || '') + '">' +
