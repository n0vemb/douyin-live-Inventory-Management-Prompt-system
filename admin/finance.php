...
?>
        ...
        <div class="card" style="margin-bottom:18px;">
            <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
                ...
                <span style="font-size:13px; color:var(--text-secondary); white-space:nowrap;">平台</span>
                <select id="platformFilter" class="form-input" style="width:110px;" onchange="loadFinance()">
                    <option value="">全部</option>
                </select>
                <span style="font-size:13px; color:var(--text-secondary); white-space:nowrap;">账号</span>
                <select id="accountFilter" class="form-input" style="width:110px;" onchange="loadFinance()">
                    <option value="">全部</option>
                </select>
                <span style="font-size:13px; color:var(--text-secondary); white-space:nowrap;">备注</span>
        ...
        (function() {
            const now = new Date();
            document.getElementById('dateTo').value = localDate(now);
            const ago = new Date(now - 30*86400000);
            document.getElementById('dateFrom').value = localDate(ago);
        })();
        ...
        function setPreset(days) {
            ...
            document.getElementById('dateTo').value = localDate(now);
            if (days === 'this_month') {
                document.getElementById('dateFrom').value = localDate(new Date(now.getFullYear(), now.getMonth(), 1));
                document.querySelector('[data-days="this_month"]').classList.add('active');
            } else if (days === 'last_month') {
                const last = new Date(now.getFullYear(), now.getMonth() - 1, 1);
                const lastEnd = new Date(now.getFullYear(), now.getMonth(), 0);
                document.getElementById('dateFrom').value = localDate(last);
                document.getElementById('dateTo').value = localDate(lastEnd);
                document.querySelector('[data-days="last_month"]').classList.add('active');
            } else {
                document.getElementById('dateFrom').value = localDate(new Date(now - days*86400000));
        ...
        async function loadFinance() {
            ...
            try {
                ...
                const pfVal = pfSel.value;
                const platforms = [...new Set(data.data.batches.map(b => b.platform || '').filter(p => p))];
                pfSel.innerHTML = '<option value="">全部</option>' + platforms.map(p => '<option value="' + escHtml(p) + '"' + (pfVal === p ? ' selected' : '') + '>' + escHtml(p) + '</option>').join('') + '<option value="__empty__"' + (pfVal === '__empty__' ? ' selected' : '') + '>空</option>';

                // 填充账号下拉
                const acSel = document.getElementById('accountFilter');
                const acVal = acSel.value;
                const accounts = [...new Set(data.data.batches.map(b => b.account || '').filter(a => a))];
                acSel.innerHTML = '<option value="">全部</option>' + accounts.map(a => '<option value="' + escHtml(a) + '"' + (acVal === a ? ' selected' : '') + '>' + escHtml(a) + '</option>').join('') + '<option value="__empty__"' + (acVal === '__empty__' ? ' selected' : '') + '>空</option>';

                // 填充备注下拉
                const rmSel = document.getElementById('remarkFilter');
                const rmVal = rmSel.value;
                const remarks = [...new Set(data.data.batches.map(b => (b.remark || '').trim()).filter(r => r))];
                rmSel.innerHTML = '<option value="">全部</option>' + remarks.map(r => '<option value="' + escHtml(r) + '"' + (rmVal === r ? ' selected' : '') + '>' + escHtml(r) + '</option>').join('') + '<option value="__empty__"' + (rmVal === '__empty__' ? ' selected' : '') + '>空</option>';
                ...
                if (pfVal === '__empty__') {
                    ...
                    filtered = filtered.filter(b => (b.platform || '') === pfVal);
                }
                if (acVal === '__empty__') {
                    filtered = filtered.filter(b => !(b.account || ''));
                } else if (acVal) {
                    filtered = filtered.filter(b => (b.account || '') === acVal);
                }
                if (rmVal === '__empty__') {
                    filtered = filtered.filter(b => !(b.remark || '').trim());
                } else if (rmVal) {
                    filtered = filtered.filter(b => (b.remark || '').trim() === rmVal);
                }
